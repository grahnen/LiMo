...
public:
	...
	{
		return itv1.ubound < itv2.ubound;
	}
};
...
void StackUnknownMonitor::do_linearization()
{
	...
	try
	{
		...
		std::set<AtomicInterval, comparatorItv> pendingPushSorted;
		...
		for (auto &itv : pendingPushSorted)
		{
			...
			timestamp_t lastValid = findLastValid(history.completedValues,
												  history.emptyPop,
												  itv, history.crashedPops,
												  max_ts);
			if(verbose)
			{
				*output << "Last Valid returned " << lastValid <<"\n";
			}
			if(lastValid == -1)
			{
				throw Violation("no valid pop");
			}
			auto popItr = history.crashedPops.find(lastValid);
			...
			while (feasibleTree.elementRight(lastFeasible) > 0)
			{
				...
				popItr++;
				if (popItr == history.crashedPops.end())
				{
