...
void StackUnknownHistory::add_push_call(event_t &call)
{
	if (!call.val)
		throw Violation("push called without value " + ext2str(call));
	activeOperations[call.thread] = call;
	times.push_back(call.timestamp);
	events[call.timestamp] = call;
}

void StackUnknownHistory::add_pop_call(event_t &call)
{
	activeOperations[call.thread] = call;
	times.push_back(call.timestamp);
	events[call.timestamp] = call;
	pop_call.insert(call.timestamp);
}

using LinRes = History<StackUnknownHistory>::LinRes;
LinRes StackUnknownHistory::add_ret(event_t &ret, bool crash = false)
{
	times.push_back(ret.timestamp);
	events[ret.timestamp] = ret;
...
void StackUnknownHistory::add_crash(event_t &crash)
{
	times.push_back(crash.timestamp);
	events[crash.timestamp] = crash;
	pendingPops.push_back({});
	for (auto [tid, event] : activeOperations)
	{
		...
		else if (event.type == Epop)
		{
			...
			pendingPops[currentEra].push_back(AtomicInterval::closed(event.timestamp, crash.timestamp));
		}
	}
	currentEra++;
	crashes.insert(crash.timestamp);

	activeOperations.clear();
}

bool StackUnknownHistory::complete() const
{
	return activeOperations.size() == 0;
}

void StackUnknownHistory::simplify()
{
	if (!complete())
	{
		throw Exception("simplifying incomplete history");
	}
...
StackUnknownHistory::LinRes StackUnknownHistory::step()
{
	return LinRes();
}

std::map<timestamp_t, event_t> StackUnknownHistory::createTimestampedHistory(tid_t &numThreads, timestamp_t &max_ts)
{
