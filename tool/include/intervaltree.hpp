...
inline bool contains(ItvTree::Node *t, AtomicInterval i) {
    ...
    if( ii <= L && R <= jj) return true;
    if( t->left != nullptr && ii <= L && R <= t->left->data.k ) return true;
    if( t->left != nullptr && ii <= L && R > t->left->data.k ) return contains(t->right, i);
    if( L <= ii ) return contains(t->left, i);
    return false;
}
