...
class StackUnknownHistory : public History<StackUnknownHistory>
{
private:
	int currentEra = 0;
...
public:
	...
	std::vector<std::vector<AtomicInterval>> pendingPops;
	std::set<AtomicInterval> emptyPop;
	
	std::vector<timestamp_t> times;
	std::map<timestamp_t, event_t> events;

	std::set<timestamp_t> push_ret;
	std::set<timestamp_t> pop_call;
	std::set<timestamp_t> crashes;
