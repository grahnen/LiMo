...
#define MESSAGE_AUTOMATA_FAIL 6
...
struct automata_event {
    std::string tp;
    std::string op;
    int val;
    automata_event(std::string tp, std::string op, int val) : tp(tp), op(op), val(val) {
...
struct transition {
    automata_event req;
    std::string trg;
    transition(std::string tp, std::string op, int val, std::string trg) : req(tp, op, val), trg(trg) {}
};

bool matches(automata_event req, automata_event arg) {
    bool okT = req.tp == "any" || req.tp == arg.tp;
    bool okO = req.op == "any" || req.op == arg.op;
    bool okA = req.val == arg.val;
    return okT && okO && okA;
}

std::map<std::string, std::vector<transition>> automata = {
{"q0", {
    transition("call", "push", 1, "q0"),
    transition("ret", "push", 1, "q1"),
    transition("any", "any", 3,"q0")
}
},
{"q1", {
transition("call", "push", 2, "q1"),
transition("ret", "pop", 1, "q1"),
transition("ret", "push", 1, "q11"),
transition("ret", "push", 2, "q11"),
transition("call", "pop", 2, "q2"),
transition("any", "any", 3, "q1")
}
},

{"q11",  {
transition("call", "pop", 1, "q1"),
transition("call", "pop", 2, "q1"),
transition("any", "any", 3, "q11")
}
},
{"q2", {
        transition("any", "any", 3, "q2"),
        transition("ret", "pop", 2, "q2")
}
}
};
...
struct partitioning {
    ...
    partitioning() {}
    partitioning(std::set<int> one, std::set<int> two, std::set<int> three) : one(one), two(two), three(three) {}
    ...
    int operator()(int arg){
        ...
        if(two.contains(arg))
            return 2;
        if(three.contains(arg))
            return 3;

        throw std::logic_error("Arg " + ext2str(arg) + " not contained: ");
    }
};


std::vector<partitioning> get_partitions(int n_elems){
    std::vector<partitioning> Ps;
    if(n_elems == -1)
        return { partitioning() };

    int head = n_elems;
    int tail = n_elems - 1;
    std::vector<partitioning> partitions = get_partitions(tail);
    for(auto p : partitions) {
        std::set oc = std::set(p.one);
        std::set tc = std::set(p.two);
        std::set thc = std::set(p.three);
        oc.insert(head);
        tc.insert(head);
        thc.insert(head);
        Ps.push_back(partitioning(oc, p.two, p.three));
        Ps.push_back(partitioning(p.one, tc, p.three));
        Ps.push_back(partitioning(p.one, p.two, thc));
    }

    return Ps;
}

std::set<std::string> next_steps(std::string src, automata_event ev) {
    std::set<std::string> next;
    for( transition &it : automata[src] ) {
        if(matches(it.req, ev))
            next.insert(it.trg);
    }
...
bool reaches_q2(std::string curr, std::vector<automata_event> evs) {
    if(evs.size() == 0)
        return curr == "q2";

    automata_event e = evs[0];
    std::vector<automata_event> tail;
    if(evs.size() > 1)
        tail = std::vector(evs.begin() + 1, evs.end());

    std::set<std::string> next = next_steps(curr, e);
    for(std::string n : next) {
        bool r = reaches_q2(n, tail);
        if(r)
            return true;
    }
...
std::vector<automata_event> aut_evs_from_ints(partitioning P, int size, std::vector<int> int_h) {
    std::vector<int> state;
    state.resize(size, 4);

    std::vector<automata_event> out;
    for(auto i : int_h) {
        switch(state[i]) {
            case 4:
                out.push_back(automata_event("call", "push", P(i)));
                break;
            case 3:
                out.push_back(automata_event("ret", "push", P(i)));
                break;
            case 2:
                out.push_back(automata_event("call", "pop", P(i)));
                break;
            case 1:
                out.push_back(automata_event("ret", "pop", P(i)));
                break;
        }
  ...
  if(rank == 0) {
      ...
      if(flag) {
        ...
        if(tag == MESSAGE_MISMATCH) {
          ...
          crashes++;
            ...
            std::cout << "Automata failure!" << std::endl;
            std::vector<int> crash;
            crash.resize(n_elements * 4);
            MPI_Recv(&crash[0], n_elements * 4, MPI_INT, from, tag, MPI_COMM_WORLD, &status);
    ...
    while(fresh_init) {
      ...
      if(msg == STATUS_DONE) {
        ...
        std::vector<partitioning> Ps = get_partitions(n_elements);
        while(generator) {
          ...
          for(auto P : Ps) {
              std::vector<automata_event> autEvs = aut_evs_from_ints(P, n_elements, int_h);
              if(reaches_q2("q0", autEvs)) {
                  rq2 = true;
                  break;
              }
          }
          ...
          if((res == MatchLin && rq2) ) { // || (res == MatchViol && !rq2)) {
              MPI_Send(int_h.data(), int_h.size(), MPI_INT, 0, MESSAGE_AUTOMATA_FAIL, MPI_COMM_WORLD);
          }
