...
with open('cover_result.csv', 'r') as f:
    for l in f.readlines():
        res = parse_result(l)
        if res is None:
            continue
        save_result('Cover', *res)

with open('stack_result.csv', 'r') as f:
    for l in f.readlines():
        res = parse_result(l)
        if res is None:
            continue

        save_result('Stack', *res)
...
for impl, res in data.items():
    for alg, res2 in res.items():
        for thrs, res3 in res2.items():
            print(impl, alg, thrs)
            xaxis = sorted([r for r,v in res3.items()])
            ...
            c_a = 0.6 if alg == 'Cover' else 0.0
            c_b = 0.0 if alg == 'Cover' else 0.6
            c_c = 0.1 + 0.8 * thrs * (1.0/64.0)
            clr = (c_a, c_b, c_c, 1.0)
            p = axes[impl].plot(xaxis, yrel, color=clr, linewidth=0.5, label=f'{alg} {thrs}t')
            print(p)
            handles.extend(p)
...
handles, labels = axes['tsstack'].get_legend_handles_labels()
print(labels)
print(labels)
...
plt.savefig(f"comprehensive.png", dpi=300)
